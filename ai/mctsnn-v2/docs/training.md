...
for gen in range(N_GENS):
    ...
    # 4. GATE new vs old  (implemented: gating=True)
    #    Self-play data is generated by the INCUMBENT; the trained candidate must score
    #    >= GATE_THRESHOLD over GATE_GAMES greedy games vs it to be promoted to data
    #    generator. The anti-collapse ratchet: a drifting candidate can't poison future
    #    data. Gate games double as the held-out validation set.
    if gate_score(candidate, incumbent) >= GATE_THRESHOLD:
        incumbent = candidate
```
...
| Replay buffer | `src/replay_buffer.py` (FIFO ring) | ✅ exists |
| Trainer (encode + losses + optimizer) | `src/trainer.py` (`run_training`) | ✅ exists |
| Checkpoint publish/load | `{"net":…, "policy":…}` dict; loader in `run_games.py` | ✅ load side exists |
| Gating (new vs old) | `run_training(gating=True)` — candidate vs incumbent, greedy | ✅ exists |
...
**Loop-level anti-collapse machinery** (see [endturn_collapse.md](endturn_collapse.md)
for the failure modes these answer):
...
- **Policy target pruning** (`prune_targets` in the search): see
  [mcts.md](mcts.md) — removes the exploration-noise visit floor from recorded targets.
- **LR cosine decay** (`lr_final`): damps late-run value sloshing once the curriculum cap
  dwells.
- **Horizon discipline**: margin terms that saturate (exploration once the map is fully
  revealed) stop differentiating outcomes at long caps and the labels collapse into
  noise — cap the curriculum at a horizon the current signal can still grade
  (the mix03 run collapsed within ~5 gens of crossing cap 13; see the fix log).
